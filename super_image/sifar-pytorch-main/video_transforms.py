...
class GroupTrivialAugment():
    def __init__(self):
        self.worker = aug_lib.TrivialAugment()
...
class GroupErase:
    def __init__(self):
        self.worker = iaa.Cutout(fill_mode="gaussian", fill_per_channel=True,size=0.2,nb_iterations=(1, 5))

    def __call__(self, img_group):
        return self.worker(images=img_group)

class GroupSharpen(object):
    def __init__(self):
        self.worker = iaa.convolutional.Sharpen(alpha=0.5,lightness=1.0)
...
class GroupContrast(object):
    def __init__(self):
        self.worker = iaa.contrast.GammaContrast(0.33)

    def __call__(self, img_group):
        return self.worker(images=img_group)


class GroupDrop(object):
    def __init__(self):
        self.worker = iaa.TotalDropout(0.5)

    def __call__(self, img_group):

        return self.worker(images=img_group)

class GroupBlur(object):
    def __init__(self):
        self.worker = iaa.Sometimes(0.33,
    ...
    def __call__(self, img_group):
        ...
        return self.worker(images=img_group)
        # img_group = self.worker.augment_images(images=img_group)
        # for img in img_group:
        #     img.save("./image/"+str(i)+".jpg")
        #     i+=1

class img2ndarry(object):
    def __call__(self, img_group):
     return [np.array(img) for img in img_group]

class ndarry2img(object):
    def __call__(self, img_group):
...
class GroupOverSample(object):
    ...
    def __call__(self, img_group):
        ...
        # print(len(oversample_group))
        # print(oversample_group)
...
if __name__ == "__main__":
    ...
    im = Image.open('/home/data1/qxh/code/super-image/3010/1.jpg')
