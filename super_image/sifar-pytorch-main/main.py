        ...
        for x in range(0,1):
            for images,filename in data_loader_test:
                ...
                for i in range(0,batch_size):
                    print(filename[i])
                    if x == 0:
                        outputs[filename[i]] = output[i].tolist()
                    elif x<4:
                        tmp = torch.tensor(outputs[filename[i]]).cuda() + output[i]
                        outputs[filename[i]] = tmp.tolist()
                    elif x == 4:
                        tmp = (torch.tensor(outputs[filename[i]]).cuda() + output[i])/5
                        outputs[filename[i]] = tmp.tolist()
        with open(str(output_dir)+'/test_result.json', 'w') as f:
            json.dump(outputs, f)
        print("eval done")
