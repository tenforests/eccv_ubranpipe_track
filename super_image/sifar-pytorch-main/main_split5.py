...
import simclr
from engine import train_one_epoch, evaluate, concat_all_gather
...
from video_dataset import VideoDataSet, VideoDataSetLMDB, VideoDataSetOnline,  TestDataSet,NineFramesDataSet,VideoDatasetByCV,VideoDatasetByDecord,VideoDataset3
...
from newModels import NewModel,MLDECORDModel
...
def main(args):
    ...
    tmp1 = train_list_name
    tmp2 = val_list_name
    for  i in range(begin,end):

        train_list_name = "split" + str(i)+"_"+tmp1
        val_list_name = "split" + str(i)+"_"+tmp2
        # print(type(num_classes))
        # print("step1",num_classes, train_list_name, val_list_name, test_list_name, filename_seperator, image_tmpl, filter_video, label_file)
    ...
    #    std = IMAGENET_DEFAULT_STD
        ...
        # max_accuracy = 0.0
        mAP = 0.0
        output_dir = Path(args.output_dir+"/split"+str(i)+"/")
        if args.initial_checkpoint:
            print("Loading pretrained model")
            if Path(args.initial_checkpoint).stem == "sifar_base_patch4_window14_224_3x3-kinetics400_f8_pe_aug":
                checkpoint = torch.load(args.initial_checkpoint, map_location='cpu')
            else:
                checkpoint = torch.load(args.initial_checkpoint+"split"+str(i)+"/model_best.pth", map_location='cpu')
        ...
        # print("train_list",train_list,args.data_dir)
        train_augmentor = get_augmentor(True, args.input_size,mean= mean , std=std, threed_data=args.threed_data,
                                        version=args.augmentor_ver, scale_range=args.scale_range, dataset=args.dataset)
        dataset_train = NineFramesDataSet(args.data_dir, train_list, args.duration, args.frames_per_group,
                                       num_clips=args.num_clips,
                                       modality=args.modality, image_tmpl=image_tmpl,
                                       dense_sampling=args.dense_sampling,
                                       transform=train_augmentor, is_train=True, test_mode=False,
                                       seperator=filename_seperator, filter_video=filter_video,num_classes=num_classes)
        ...
        dataset_val =  NineFramesDataSet(args.data_dir, val_list, args.duration, args.frames_per_group,
                                     num_clips=args.num_clips,
                                     modality=args.modality, image_tmpl=image_tmpl,
                                     dense_sampling=args.dense_sampling,
        ...
        if args.eval:
            # 重写eval模型，直接输出json文件 zt

            test_list = os.path.join(args.data_dir,"test.txt")

            dataset_test = NineFramesDataSet(args.data_dir, test_list, args.duration, args.frames_per_group,
                                         num_clips=args.num_clips,
                                         modality=args.modality, image_tmpl=image_tmpl,
                                         dense_sampling=args.dense_sampling,
                                         transform=val_augmentor, is_train=False, test_mode=True,
                                         seperator=filename_seperator, filter_video=filter_video, num_classes=num_classes)

            data_loader_test = build_dataflow(dataset_test, is_train=False, batch_size=args.batch_size,
                                             workers=args.num_workers, is_distributed=args.distributed)
            torch.no_grad()
            model.eval()
            outputs = {}
            # print(type(outputs))
            for x in range(0,1):
                for images,filename in data_loader_test:
                    ...
                    output = output.reshape(batch_size, args.num_crops * args.num_clips, -1).mean(dim=1)
                    if args.distributed:
                        output = concat_all_gather(output)
                    output = torch.nn.functional.sigmoid (output)
                    for j in range(0,batch_size):
                        # print(output[i].tolist(),filename[i])
                        if x == 0:
                            outputs[filename[j]] = output[j].tolist()
                        elif x < 4:
                            tmp = torch.tensor(outputs[filename[j]]).cuda() + output[j]
                            outputs[filename[j]] = tmp.tolist()
                        elif x == 4:
                            tmp = (torch.tensor(outputs[filename[j]]).cuda() + output[j]) / 5
                            outputs[filename[j]] = tmp.tolist()
            with open(str(output_dir)+'/model_best_test_result.json', 'w') as f:
                json.dump(outputs, f)
            print("eval done")


            # test_stats = evaluate(data_loader_test, model, device, num_tasks, distributed=True, amp=args.amp, num_crops=args.num_crops, num_clips=args.num_clips)
            # print(f"Accuracy of the network on the {len(dataset_val)} test images: {test_stats['acc1']:.1f}%")
            # print(f"mAP of the network on the {len(dataset_test)} test images: {test_stats['mAP']:.1f}%")
            if i!=end-1:
                continue
            else:
                return
        ...
        print(f"Start training, currnet mAP is {mAP:.2f}")
        ...
        for epoch in range(args.start_epoch, args.epochs):
            ...
            mAP = max(mAP, test_stats["mAP"])
            print(f'best mAP: {mAP:.2f}%')

            if args.output_dir:
                checkpoint_paths = [output_dir / ('epoch'+str(epoch)+'_checkpoint.pth')]
                # if test_stats["acc1"] == max_accuracy:
                #     checkpoint_paths.append(output_dir / 'model_best.pth')
                if test_stats["mAP"] == mAP:
                    checkpoint_paths.append(output_dir / 'model_best.pth')
