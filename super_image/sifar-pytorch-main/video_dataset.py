...
def random_clip(video_frames, sampling_rate, frames_per_clip, fixed_offset=False, start_frame_idx=0,
                end_frame_idx=None):
    """
...
def sample_train_clip(video_length, num_consecutive_frames, num_frames, sample_freq, dense_sampling, num_clips=1):
    ...
    if dense_sampling:
        ...
        if num_clips == 1:  # backward compatibility
            ...
            for i in range(num_clips):
                if frames_per_segment <= 0:
                    ...
                    frame_idx[i] = np.asarray(
                        random_clip(max_frame_idx, sample_freq, num_frames, False, i * frames_per_segment,
        ...
        # print("ave_frames_per_group",ave_frames_per_group)
        # print(sample_freq)
...
class VideoDataSet(data.Dataset):
    ...
    def remove_data(self, idx):
        ...
        print("Original videos: {}\t remove {} videos, remaining {} videos".format(original_video_num, len(idx),
                                                                                   len(self.video_list)))
    ...
    def __getitem__(self, index):
        ...
        for modality, video_dataset in zip(self.modality, self.video_datasets):
            if indices is None:
                ...
                indices = video_dataset._sample_indices(
                    record) if video_dataset.is_train else video_dataset._get_val_indices(record)
...
class VideoDataSetOnline(VideoDataSet):
    ...
    def get_data(self, record, indices):
        ...
        print("file",os.path.join(self.root_path, record.path))
...
class TestDataSet(VideoDataSet):
    def __init__(self, root_path, list_file, num_groups=8, frames_per_group=1, sample_offset=0,
                 num_clips=1, modality='rgb', dense_sampling=False, fixed_offset=True,
                 image_tmpl='{:05d}.jpg', transform=None, is_train=True, test_mode=True, seperator=' ',
                 filter_video=0, num_classes=None, whole_video=False,
                 fps=29.97, audio_length=1.28, resampling_rate=24000):
        super().__init__(root_path, list_file, num_groups, frames_per_group, sample_offset,
                         num_clips, modality, dense_sampling, fixed_offset,
    ...
    def __getitem__(self, index):
        ...
        # zt 版本加载数据
        path1 = os.path.join(self.root_path, record.path)
        images = []
        for i in range(0, 9):
            imagePath = os.path.join(path1, str(i) + ".jpg")
            img_tmp = Image.open(imagePath)
            img = img_tmp.copy()
            img_tmp.close()
            images.append(img)
        # images = self.get_data(record, indices)
        ...
        # re-order data to targeted format.
        return images, record.path


# 九图数据加载 zt
class NineFramesDataSet(VideoDataSet):
    def __init__(self, root_path, list_file, num_groups=8, frames_per_group=1, sample_offset=0,
                 num_clips=1, modality='rgb', dense_sampling=False, fixed_offset=True,
                 image_tmpl='{:05d}.jpg', transform=None, is_train=True, test_mode=True, seperator=' ',
                 filter_video=0, num_classes=None, whole_video=False,
...
# 新的流数据加载器 基于opencv
class VideoDatasetByCV(VideoDataSet):
    def __init__(self, root_path, list_file, num_groups=8, frames_per_group=1, sample_offset=0,
                 num_clips=1, modality='rgb', dense_sampling=False, fixed_offset=True,
                 image_tmpl='{:05d}.jpg', transform=None, is_train=True, test_mode=True, seperator=' ',
                 filter_video=0, num_classes=None, whole_video=False,
    ...
    # images = self.transform(images)
    # label = self.get_label(record)
    # 用opencv重写 //indices得到相应的下标，test

    def __getitem__(self, index):
        record = self.video_list[index]
        max_frame_idx = record.num_frames
        num_frames = 9
        ave_frames_per_group = max_frame_idx // num_frames
        frame_idx = np.arange(0, num_frames) * ave_frames_per_group
        # frame_idx = np.repeat(frame_idx, repeats=1)
        offsets = np.random.uniform(low=0.0,high=float(ave_frames_per_group),size=num_frames)
        offsets = offsets.astype(int)
        frame_idx = frame_idx + offsets
        video = os.path.join(self.root_path, record.path)
...
class VideoDatasetByDecord(VideoDataSet):
        def __init__(self, root_path, list_file, num_groups=8, frames_per_group=1, sample_offset=0,
                     num_clips=1, modality='rgb', dense_sampling=False, fixed_offset=True,
                     image_tmpl='{:05d}.jpg', transform=None, is_train=True, test_mode=True, seperator=' ',
                     filter_video=0, num_classes=None, whole_video=False,
        ...
        def __getitem__(self, index):
            ...
            # frame_idx = np.arange(0, num_frames) * ave_frames_per_group
            # frame_idx = np.repeat(frame_idx, repeats=1)
            idx = np.random.uniform(low=0.0, high=float(max_frame_idx), size=num_frames)
            idx = idx.astype(int)
            # frame_idx = frame_idx + offsets
            decord.bridge.set_bridge('torch')
            video = os.path.join(self.root_path, record.path)
            video = VideoReader(video, ctx=cpu(0))
            frames = video.get_batch(idx)
...
class VideoDataset3(VideoDataSet):
    def __init__(self, root_path, list_file, num_groups=8, frames_per_group=1, sample_offset=0,
                 num_clips=1, modality='rgb', dense_sampling=False, fixed_offset=True,
                 image_tmpl='{:05d}.jpg', transform=None, is_train=True, test_mode=True, seperator=' ',
                 filter_video=0, num_classes=None, whole_video=False,
    ...
    def __getitem__(self, index):
            ...
            for i in range(1,28,3):
                ...
                imagePath = os.path.join(path1, str(tmp) + ".jpg")
                img_tmp = Image.open(imagePath)
                img = img_tmp.copy()
                img_tmp.close()
                images.append(img)
